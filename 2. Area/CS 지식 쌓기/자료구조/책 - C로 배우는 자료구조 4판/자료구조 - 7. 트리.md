...
5. [[#이진 탐색 트리(BST, Binary Search Tree)|이진 탐색 트리(BST, Binary Search Tree)]]
	1. [[#이진 탐색 트리(BST, Binary Search Tree)#개념|개념]]
	2. [[#이진 탐색 트리(BST, Binary Search Tree)#탐색 연산|탐색 연산]]
	3. [[#이진 탐색 트리(BST, Binary Search Tree)#삽입 연산|삽입 연산]]
	4. [[#이진 탐색 트리(BST, Binary Search Tree)#삭제 연산|삭제 연산]]
	...
	5. [[#이진 탐색 트리(BST, Binary Search Tree)#이진 탐색 트리의 구현|이진 탐색 트리의 구현]]
6. [[#균형 이진 탐색 트리(AVL)|균형 이진 탐색 트리(AVL)]]
	1. [[#균형 이진 탐색 트리(AVL)#개념|개념]]
	2. [[#균형 이진 탐색 트리(AVL)#AVL 트리의 개념과 유형|AVL 트리의 개념과 유형]]
	3. [[#균형 이진 탐색 트리(AVL)#AVL 트리의 회전 연산|AVL 트리의 회전 연산]]
	...
	4. [[#균형 이진 탐색 트리(AVL)#AVL 트리의 구현|AVL 트리의 구현]]
7. [[#힙Heap의 개념과 연산 및 구현|힙Heap의 개념과 연산 및 구현]]
	1. [[#힙Heap의 개념과 연산 및 구현#힙의 개념|힙의 개념]]
	2. [[#힙Heap의 개념과 연산 및 구현#힙의 추상 자료형|힙의 추상 자료형]]
	3. [[#힙Heap의 개념과 연산 및 구현#힙의 삽입 연산|힙의 삽입 연산]]
		1. [[#힙의 삽입 연산#예시 힙 삽입 동작|예시 힙 삽입 동작]]
	4. [[#힙Heap의 개념과 연산 및 구현#힙의 삭제 연산|힙의 삭제 연산]]
		1. [[#힙의 삭제 연산#예시 힙 삭제 연산|예시 힙 삭제 연산]]
	5. [[#힙Heap의 개념과 연산 및 구현#힙의 구현|힙의 구현]]
8. [[#트리를 어떤 자료구조로 구현하는가?와 그 이유|트리를 어떤 자료구조로 구현하는가?와 그 이유]]
9. [[#생각해 볼 만한 질문들|생각해 볼 만한 질문들]]
...
- `switch expression` 사용. [[Csharp - switch(Statement, Expression)|switch]] 참고.
...
- 이해
	- **$O(h)$는 트리가 균형잡혀 있을 때의 이야기**이고, 삽입 순서에 따라 편향 트리가 되면 h가 n에 가까워져서 worst case에서 $O(n)$로 연결 리스트와 똑같게 된다.
### 탐색 연산
1. 루트에서 시작한다.
...
### 힙의 개념
- 완전 이진 트리이면서, 모든 부모 - 자식 쌍이 특정 대소 관계를 만족하는 트리.
	- `최대 힙Max Heap` : 부모값 >= 자식값, 루트는 항상 최댓값
	- `최소 힙Min Heap` : 부모값 <= 자식값, 루트는 항상 최솟값

- 이진 탐색 트리와의 차이점
	- 이진 탐색 트리는 `왼쪽 < 부모 < 오른쪽`이라는 **형제 간 순서까지 포함**한 전체 정렬 관계
	- 히프는 부모 - 자식 간의 관계만 있고 형제 간에는 순서가 없다.

- 히프에서의 탐색 : $O(n)$  
- 히프에서의 최댓값 / 최솟값을 꺼내는 연산 : $O(log\,n)$

- 완전 이진 트리라는 조건 덕분에 이진 트리의 구현에서 다룬 배열 기반의 표현이 그대로 잘 들어맞는다.
...
### 힙의 삽입 연산
1. 완전 이진 트리 조건을 유지해야 하므로, 새 노드는 항상 배열의 다음 빈 자리에 넣는다.
2. 새로 넣은 노드를 부모와 비교, 힙 조건이 깨져 있으면 둘을 교환한다.
3. 교환 후에도 부모와 비교하며 이 과정을 반복한다. `상향 이동, sift-up`
	- 힙 조건을 만족하거나 루트에 도달하면 멈춘다.
...
graph TD
    ...
    N85 --> N70["70"]
```
1. `85`는 `70`의 오른쪽 자식으로 들어감
2. 부모 `70`보다 크니까 교환한다. `85` -> `70`으로 올라가고, `70`은 밑으로 내려간다. 
3. 다시 부모 `90`과 비교, `90`이 더 크므로 이 상태를 유지한다.

### 힙의 삭제 연산
- **항상 루트만 삭제**한다. (최댓값, 최솟값을 꺼내는 것 자체가 삭제 연산)
1. 루트 값을 결과만 따로 저장해둔다.
2. 마지막 노드(배열의 맨 끝)를 루트 자리로 옮기고, 해당 마지막 자리는 비운다. 
	- 완전 이진 트리 형태를 유지하기 위함
3. 새로 루트가 된 노드를 자식들과 비교한다 
	- 힙 조건이 깨져 있으면 더 큰(작은) 자식과 교환한다.
4. 교환 후 그 자리에서 다시 자식들과 비교하며 반복한다`하향 이동, sift-down`
	- 힙 조건을 만족하거나 자식이 없는 자리에 도달하면 멈춘다.
...
```
1. 루트 값 `90`이 꺼내짐
2. 마지막 노드 `70`을 루트로 옮김
3. `70`은 두 자식 `80`, `85` 중 더 큰 값(최대 힙 기준)과 교환하며 내려감
4. `85` 자리에 들어간 `70`은 남은 자식인 `50`과 비교, `50`보다는 크므로 멈춘다.

### 힙의 구현
- 최대 힙 기준.
- 이진 트리와 마찬가지로 인덱스는 1부터 시작한다.
- 삽입과 삭제에는 각각 `sift-up`과 `sift-down`을 적용한다.
...
public class MaxHeap<T> : IHeap<T> where T : IComparable<T>
{
	...
	{
		nodes = new T[capacity + 1]; // 인덱스 1부터 시작함 = 루트 노드의 인덱스는 1
		count = 0;
	}
	...
	public T Peek()
	{
		if (IsEmpty) throw new InvalidOperationException("힙이 비어있음");
		return nodes[1];
	}
	...
	public SiftDown(int index)
	{
		while (true)
		{
			// 완전 이진 트리이므로 이 조건을 사용할 수 있음
			int left = 2 * index;
			int right = 2 * index + 1;
			int largest = index;
			
			// 왼쪽, 오른쪽 중 큰 값 선택
			if (left <= count && nodes[left].CompareTo(nodes[largest]) > 0)
				largest = left;
			if (right <= count && nodes[right].CompareTo(nodes[largest]) > 0)
				largest = right;
...
- [[Csharp - 튜플 분해 | 튜플 분해]]는 링크 참조.

## 트리를 어떤 자료구조로 구현하는가?와 그 이유
- 이진 트리 : 둘 다 가능
- 이진 탐색 트리 : 연결 자료 구조
- 힙 : 순차 자료 구조

>[!note]
>"**이 트리가 항상 완전 이진 트리 형태를 유지하는가?**"가 핵심이다.
>- 유지하면 배열로 구현(힙)
>- 아니라면 연결 자료 구조로 구현(이진 탐색 트리)

- **이진 탐색 트리**를 연결 자료 구조로 구현한 이유
	- 값의 대소 관계만 지킬 뿐, **완전 이진 트리라는 보장이 없다.** 삽입 순서에 따라 모양이 바뀌며, 최악의 경우 편향 트리가 되기도 한다.
	- 그렇기에 배열로 표현하면 인덱스가 트리의 실제 모양에 종속되어서, 편향되는 경우 낭비되는 칸이 기하급수적으로 늘어난다. $h -> 2^h$ 
	- 연결 자료 구조는 노드 하나하나가 자기 위치를 포인터로만 표현하므로, 트리 모양에 관계 없이 낭비가 없다. 

- **힙**을 순차 자료 구조로 구현하는 이유
	- **정의 자체에 완전 이진 트리 조건이 있으며, 삽입, 삭제 연산도 그 조건을 절대 깨지 않는다.**
		- 삽입 : 항상 마지막 빈 자리에 넣고 재정렬
		- 삭제 : 루트를 지우고 마지막 노드를 루트에 넣고 재정렬
	- 그래서 `2i, 2i+1, i/2` 조건이 항상 유효하며, 낭비되는 빈 칸도 없다.
	- 임의의 값을 찾아 들어가는 탐색 없이, **루트와 마지막 자리**라는 2개의 고정된 위치만 다루므로, 연결 구조의 장점(포인터 재배선의 유연함)이 아예 쓸모가 없다.

## 생각해 볼 만한 질문들

1. **일반 트리와 이진 트리의 차이는?**
	- 이진 트리는 자식 수가 2개로 제한되지만 일반 트리는 그렇지 않음
	- 이진 트리의 자식에는 왼쪽, 오른쪽이라는 순서가 있기도 하다.

2. **이진 트리와 이진 탐색 트리의 차이점?**
	- 이진 트리는 구조 조건(최대 자식 2개)만 있다. 값의 대소 관계 조건은 없다.
	- 이진 탐색 트리는 `왼쪽 < 부모 < 오른쪽`이라는 정렬 조건이 추가된다.

3. **완전 이진 트리를 배열로 표현하기 좋은 이유?**
	- 마지막 레벨까지 빈틈없이 채워진다는 조건이 있어서 `2i(왼쪽), 2i+1(오른쪽), i/2(부모)` 공식이 항상 성립한다.
	- 편향 트리를 배열로 표현하면 어떻게 되는지와 대조해서 설명할 수 있으면 좋음.
		- 편향 트리의 경우 수많은 빈 공간이 생기므로 배열로 표현하면 메모리 낭비가 심하다. 이런 경우는 연결 자료 구조로 표현하는 편이 더 좋음.

4. **이진 탐색 트리의 탐색, 삽입, 삭제 연산의 평균 시간 복잡도와 최악의 경우가 다른 이유?**
	- 세 연산 모두 루트에서 리프 방향으로 트리 높이만큼 내려가는 구조로, 같은 이유로 $O(h)$
	- 평균적으로는 $h = log\,n$ 이지만, 삽입 순서에 따라 편향 트리가 되면 `h => n`으로 가까워져서 최악의 경우 $O(n)$까지 나빠진다.
	- AVL 트리는 이 문제를 어떻게 해결하는가? 까지 연결하면..
		- BF라는 팩터를 넣고 양쪽 자식으로 이어지는 높이 차를 계산, `+-1`을 초과하면 트리를 회전시키는 방식으로 트리가 편향되는 현상을 방지한다. 

5. **AVL 트리에서 LL/RR과 LR/RL의 회전 횟수가 다른 이유?**
	- LL/RR은 한쪽으로만 치우친 형태로, 회전 한 번으로 해결된다. 
	- LR/RL은 지그재그 형태라서 한 번 회전시켜서 LL/RR 모양으로 바꾼 다음 다시 회전시켜야 함.

6. **이진 탐색 트리와 힙의 차이는?**
	- 이진 탐색 트리
		- 왼쪽, 오른쪽, 부모 전체의 정렬 관계를 유지한다. 
		- 임의 값 탐색 $O(log\,n)$이 가능.
	- 힙
		- 부모 - 자식 간의 관계만 유지하며, 형제 간의 순서는 없다.
		- 임의 값 탐색 $O(n)$. 
		- 최댓값, 최솟값 추출은 항상 $O(log \, n)$. 
			- 추출 자체는 $O(1)$이지만 힙의 정렬 연산이 들어가기 때문.
	- 우선순위 큐와 정렬된 탐색 중 각각이 어떤 걸 쓰는가?
		- **우선순위 큐 : 힙**
			- 우선순위 큐란, 전체 데이터 중 **가장 큰 값이나 작은 값**을 빠르게 찾아내고 삭제하는 것이 핵심인 자료구조이다. 
			- 힙은 최댓값, 최솟값의 데이터를 확인하는 데 $O(1)$의 시간이 걸리며, 추출 및 삽입 또한 트리의 높이만큼만 탐색하므로 $O(log \, n)$이 소요된다.
			- BST(이진 탐색 트리)도 우선 순위 큐의 역할을 할 수 있지만, 최소/최대값을 찾기 위해서는 트리의 끝까지 내려가야 하므로 $O(log\,n)$이 소요되며, 노드마다 포인터를 할당해야 하므로 메모리, 캐시 효율 측면에서 힙보다 불리하다. 
		- **정렬된 탐색 : 이진 탐색 트리**
			- **특정 값을 찾거나, 데이터들을 정렬된 순서로 순회하거나, 특정 범위의 데이터**를 찾아아 할 때는 이진 탐색 트리가 적합하다. 
			- 임의 값 탐색 : 왼쪽 자식 < 부모 < 오른쪽 자식이라는 엄격한 정렬 규칙을 유지하므로, 탐색에 $O(log\,n)$의 속도로 매우 빠르게 찾을 수 있다. 
			- 정렬 순회 및 범위 탐색 : 중위 순회를 사용해 전체 데이터를 정렬된 상태로 $O(n)$만에 읽어올 수 있다. 범위 탐색에도 용이하다.
			- 힙의 경우, 세로 방향의 느슨한 정렬만 유지하면 형제 관계에 대소 관계 규칙이 없다. 임의의 값을 찾기 위해 힙에서는 최악의 경우 모든 노드를 탐색하는 $O(n)$이 소요되며, 힙 내부의 데이터를 정렬된 상태로 순회하려면 힙 요소를 하나씩 뽑아내야 하므로(`Heap Sort`) 원본 구조가 파괴된다. 

7. **스레드 이진 트리를 쓰는 이유?**
	- 일반 연결 이진 트리의 리프 근처에 몰려 있는 `null` 링크를 순회용 스레드로 재활용
	- 재귀 ,별도 스택 없이 순회가 가능해진다. $O(h)$ -> $O(1)$

8. **이진 탐색 트리에서 차수가 2인 노드를 삭제할 때 왜 중위 후속자/선행자로 대체하는가?**
	- 해당 노드를 대체할 값은 "삭제할 노드보다 크면서 가장 작은 값"이어야 하고, 이 조건을 만족하는 게 중위 후속자.
	- 선행자로 대체해도 되지만 관례상 후속자로 대체함
	- 후속자로 값을 바꿔치고 나면, 원래의 후속자 자리를 지우는 문제는 항상 차수 0, 1 케이스로 귀결된다. 