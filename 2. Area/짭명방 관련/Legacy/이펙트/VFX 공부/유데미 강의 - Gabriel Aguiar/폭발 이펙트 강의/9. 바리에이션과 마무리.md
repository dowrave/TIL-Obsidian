- 완전 마지막 강의에 해당하는 부분인데, 센세가 스크립트로 구현해둔 부분이 있음. 
- 오브젝트의 자식에 있는 **파티클 시스템들을 일괄적으로 수정할 수 있게 구현한 스크립트**인데, 특이한 점으로는 **에디터 단위에서 클릭 한번으로 모든 자식 오브젝트를 리스트에 등록할 수 있게 해놨다**는 거임 
- 색의 경우 `Start Color`를 수정하는 스크립트로 보이며, `Color over Trail`이라든가 `Material`에 구현된 개별 요소들은 당연히 별도로 수정해야 함

> 다만 **실습을 유니티 6버전에서 하고 있기 때문에, 프로젝트가 실제로 진행된 버전과 다르다.** 오류가 발생하고 있어서 직접 적용하긴 어려워보임.

> **저런 스크립트 하나 있으면 바리에이션을 쉽게 만들 수 있다는 점에서 좋을 듯?** 모든 걸 일괄적으로 바꿀 순 없어도 스크립트로 구현하면 극히 일부 요소만 손으로 수정하면 되니까.

강의 내용은 그거 적용해서 `Green`이나 `Purple` 버전을 만드는 내용임. 여기에는 해당 스크립트만 첨부해둔다.
...
## 무엇을 더 할 수 있는가?
- 센세 유튜브 채널, Pateron 페이지, 웹사이트 등에서 센세의 이펙트 애셋을 보고 공부하기
- 본인의 작업물을 SNS에 공유하기
- `Real-Time VFX`에서 공부하거나 매 달 진행되는 챌린지에 도전하기
- 센세의 다른 강의 듣기 등등
