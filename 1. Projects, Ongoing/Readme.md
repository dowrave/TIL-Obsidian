...
- [[CSharp - 구조체 readonly 구현]]
	...
	- 크게 2가지 이유가 있는데
		1. 필드는 괜찮은데 프로퍼티에 값을 할당할 때는 `struct`가 값 타입이라서 임시 복사본을 만든 다음 저장하는 개념으로 들어가기 때문에 실질적으로는 빈 곳 에 값을 할당하게 되어서 `CS1612` 컴파일 에러가 발생.
	...
	- `readonly struct`로 구현하면
		- 프로퍼티를 구현할 필요가 없어진다. `readonly`라고 아예 박혀있으니까.
